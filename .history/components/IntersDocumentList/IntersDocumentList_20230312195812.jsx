...
const DocumentListContent = ({ type, status,student }) => {
  ...
  const downloadServer = async () => {
    ...
    var typeS = "";
    if(type === "learning Agreement After")
      typeS = "learning Agreement";
    else
      typeS = type;

    body.append("student", studentName);
    
    body.append("type", typeS.replace(" ",""));
    
    const dt = await axios.post("/api/download",body);
    console.log(dt.data)

   
    fetch("/uploads/"+dt.data.file)
      .then(response => response.blob())
      .then(blob => {
        const url = window.URL.createObjectURL(new Blob([blob]));
        const link = document.createElement('a');
        link.href = url;
        link.setAttribute('download',"/uploads/"+dt.data.file);
        document.body.appendChild(link);
        console.log(dt.data.file)
        link.click();
  ...
  const handleFormSubmit = async (event) => {
    ...
    if(type === "learning Agreement After")
      ...
      typeS = type;
    const new_file = new File([data_result],studentName + ' ' + typeS.replace(' ','') + ' '+ file.name );
