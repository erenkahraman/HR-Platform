...
const DocumentListContent = ({ type, status,student }) => {
  ...
  const downloadServer = async () => {
    ...
    const handleFileChange = (event) => {
      ...
      reader.onload = (event) => {
        const encodedData = base64.encode(event.target.result);
        setEncodedFile(encodedData);
        
        const files = new File([encodedData], 'hello.txt', { type: 'text/plain;charset=utf-8' });
       // saveAs(files);
       console.log(files)
        setfileName(files);
      };
