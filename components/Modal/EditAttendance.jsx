...
const EditAttendance = ({ intern, setModel }) => {
    ...
    const deleteDate = (date) => {
        confirmAlert({
            message: 'Are you sure you wanna delete the status: \"' + status + '\" from the date: ' + date,
            buttons: [
                {
                    label: 'Yes',
                    onClick: async () => {
                        ...
                        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                            <div class="flex justify-between items-start p-4 rounded-t border-b dark:border-gray-600">
                                <h3 class="text-xl font-semibold text-gray-900 dark:text-white">
                                    Edit Attendance for {intern.student.firstName} {intern.student.lastName}
                                </h3>
                            </div>
                            ...
                            <div class="p-6 space-y-6 flex-nowrap ">
                                ...
                                <div className="flex flex-row">
                                    ...
                                    <ul className="overflow-y-auto max-h-32 w-2/5 mb-8 space-y-4 text-left text-gray-500 dark:text-gray-400">
                                        {dates.length == 0 ?
                                            <label className="flex items-center space-x-3">No dates for this status</label>
                                            :
                                            dates.map((date) => (
                                                <li className="flex items-center space-x-3" >
                                                    <label>{date} </label> <ClearIcon color="error" fontSize="small" onClick={e => { deleteDate(date) }} /></li>
                                            ))}
                                    </ul>
